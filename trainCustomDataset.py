...
def main():
    ...
    def preprocess_train(examples):
        newExamples = {}
        images = [image["pixel_values"].convert("RGB") for image in examples]
        newExamples["pixel_values"] = [train_transforms(image) for image in images]
        newExamples["input_ids"] = tokenize_captions(examples)

        returnExamples = []
        for pixelValue,inputIds in zip(newExamples["pixel_values"],newExamples["input_ids"]):
            
            returnExamples.append({"pixel_values":pixelValue,"input_ids":inputIds})
    ...
    global DREAM_DETAIL_PRESERVATION
    global PREDICTION_TYPE
    global DREAM_TRAINING
    global SNR_GAMMA
